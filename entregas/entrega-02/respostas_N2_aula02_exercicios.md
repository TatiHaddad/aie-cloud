...
|---------|-------------------------|------------------|------------------------------|
| Produtos | Azure SQL Database | Hyperscale, 4 vCores, leitura replicada | Catálogo de 5M SKUs tem esquema fixo (preço, estoque, categoria) e exige integridade referencial com pedidos. Hyperscale escala storage até 100TB sem downtime, e réplicas de leitura suportam o alto volume de consultas do frontend e dos agentes. |
| Clientes | Azure SQL Database | Business Critical, 4 vCores, zona redundante | Dados de 50M de clientes (perfil, endereço, preferências) exigem ACID para consistência de cadastro e zona redundante para alta disponibilidade — base para faturamento e LGPD. |
| Pedidos | Azure SQL Database + Azure Service Bus | Hyperscale, 8 vCores + Service Bus Premium | 10M pedidos/mês com alta criticidade transacional exigem ACID. O Service Bus desacopla o registro do pedido do processamento downstream (pagamento, logística, notificação) via eventos. |
| Carrinhos | Azure Cosmos DB (API NoSQL) | Serverless ou autoscale 400-4000 RU/s, TTL = 24h | ~500k carrinhos com esquema variável (itens, cupons, frete calculado). O TTL nativo do Cosmos expira automaticamente os carrinhos abandonados após 24h, sem necessidade de job de limpeza. |
| Reviews | Azure Cosmos DB (API NoSQL) | Autoscale 1000-10000 RU/s, particionado por `produto_id` | 30M de textos livres com schema flexível (título, score, conteúdo, metadados). Particionamento por `produto_id` agrupa reviews do mesmo produto, otimizando a leitura por página de produto. |
| Busca de produtos | Azure AI Search | Standard S1, índice vetorial + semantic ranker | Consultas dos agentes de IA e do frontend exigem busca semântica ("cadeira confortável para trabalho") além de busca por palavra-chave. AI Search combina full-text, vector search e semantic ranking em um único serviço gerenciado. Poderia usar o SKU Free, porém para o exercício 3.1 em produção , com 5M de SKUs, a QC exigiria Standard S1 + com vector search habilitado para suportar o RAG completo. |
| Sessões | Azure Cache for Redis | Premium P1, cluster habilitado | ~1M sessões ativas exigem latência sub-milissegundo (chave-valor). Redis é o padrão de mercado para sessão de usuário, com TTL nativo e throughput muito superior ao Cosmos para esse padrão de acesso. |
| Histórico navegação | Azure Event Hubs + Azure Data Lake Storage Gen2 (via Synapse) | Event Hubs Standard (ingestão) + ADLS Gen2 em Parquet | Bilhões de eventos exigem ingestão em streaming de alto throughput (Event Hubs) e armazenamento de baixo custo em formato colunar (Parquet) para análises posteriores via Synapse Serverless, sem necessidade de banco transacional. |
| Modelos ML | Azure Blob Storage + Azure Machine Learning (Model Registry) | Container modelos (Hot tier) + AML Workspace | Modelos de recomendação, classificação e churn são artefatos versionados (.pkl, .onnx). O Model Registry do AML controla versões e linhagem e o Blob Storage hospeda os artefatos consumidos pelos endpoints de inferência. Os 3 containers desenvolvido em dev / lab (catalogo, imagens , logs) tem propósitos já definidos, os modelos exigem um quarto container dedicado (modelos), seguindo o mesmo padrão de organização por prefixo / container. |

**Bonus:** Desenhe o diagrama da camada de dados completa da QC no Excalidraw / draw.io e cole no `respostas-aula02.md`.
> 📎 Ver arquivo: `diagramas_de_camada_de_Dados.drawio` -> preciso salvar no formato correto e como png
...
R: 
| Repositório | R escolhido | Justificativa |
|---|---|---|
| **Oracle on-premise (8 TB)** | **Refactor** | Aproveitar a migração para separar os domínios monolíticos do Oracle em bases especializadas (Azure SQL para Produtos/Pedidos/Clientes, Cosmos para Carrinhos/Reviews), conforme a matriz do Exercício 2.1. Reescrever é justificado pelo ganho de escalabilidade e desacoplamento. |
| **NAS de imagens (50 TB)** | **Replatform** | As imagens em si não mudam de formato, mas o armazenamento muda de NAS (File Storage) para **Object Storage** (Blob), habilitando CDN, lifecycle policies e acesso via HTTP direto pelo catálogo — sem reescrever a aplicação que as consome, apenas trocar o protocolo de acesso. |
| **Logs em fita (200 TB)** | **Retain (temporário) → Rehost gradual** | Dado o volume e a baixíssima frequência de acesso (compliance fiscal), a migração é de menor prioridade. Inicialmente os dados permanecem retidos, sendo migrados gradualmente (ao longo dos 12 meses) para **Blob Archive tier**, que tem custo comparável ao de fitas mas com recuperação muito mais rápida em caso de auditoria. |

b) **Quais serviços Azure** ficariam com cada um, considerando custo + criticidade?
R:

| Repositório | Serviço Azure | Justificativa de custo/criticidade |
|---|---|---|
| Oracle (8 TB) → dividido | Azure SQL Database Hyperscale (Produtos/Pedidos) + Cosmos DB (Carrinhos/Reviews) | Alta criticidade transacional justifica banco gerenciado com SLA de 99,99%. Hyperscale separa custo de compute e storage, permitindo escalar cada um independentemente. |
| NAS de imagens (50 TB) | Azure Blob Storage — Hot tier inicialmente, Cool após 90 dias | Imagens recém-publicadas são acessadas com frequência (Hot); produtos descontinuados migram automaticamente para Cool via lifecycle policy, reduzindo custo em ~50%. |
| Logs em fita (200 TB) | Azure Blob Storage — Archive tier | Archive custa ~$0,002/GB/mês — para 200 TB isso representa ~$400/mês, uma fração do custo de manter e operar uma biblioteca de fitas físicas, com retrieval em horas em vez de dias. |
...
A migração será executada em **fases paralelas com cutover controlado**:

1. **Banco Oracle (8 TB):** usar o **Azure Database Migration Service (DMS)** em modo de **migração contínua (online)**. O DMS replica continuamente as alterações do Oracle on-premise para o Azure SQL via Change Data Capture (CDC), mantendo as duas bases sincronizadas durante semanas. O cutover ocorre em uma janela de minutos: pausar escritas no Oracle, aguardar a última sincronização, e redirecionar a aplicação para o Azure SQL.

2. **Imagens do NAS (50 TB):** usar o **AzCopy** com sincronização incremental (`azcopy sync`). A primeira carga (bulk) roda em background sem impactar o NAS em produção; sincronizações incrementais subsequentes copiam apenas os arquivos novos/alterados, até que o NAS e o Blob estejam idênticos — momento em que a aplicação passa a apontar para o Blob.

3. **Logs em fita (200 TB):** sem requisito de "sem downtime" (dados frios), a migração ocorre via **Azure Data Box** (dispositivo físico enviado pela Microsoft) para os primeiros grandes lotes, complementado por AzCopy para incrementos — evitando saturar o link de internet da QC com 200 TB de uma vez.


d) **Estimativa de custo de egress** para os 50 TB de imagens (a primeira saída custa banda)
R:
```
Volume: 50 TB = 51.200 GB

Egress do NAS para a internet: depende do provedor de internet local da QC
  (não é cobrado pela Azure — o egress relevante aqui é para FORA do Azure depois)

Egress de saída do Azure (ex: CDN para usuários finais) — não conta neste cálculo,
pois é tráfego normal de operação, não de migração.

Para a MIGRAÇÃO em si (entrada no Azure):
  Ingress (upload para o Blob): GRATUITO — Azure não cobra por dados entrando

Custo real da migração: zero em termos de egress Azure.
Único custo: tempo de transferência pela internet da QC (50 TB pode levar
dias/semanas dependendo da banda) — por isso a recomendação de usar
Azure Data Box para volumes grandes, evitando saturar o link corporativo.
```

> **Observação importante:** o egress só se torna um custo relevante se, no futuro, a QC precisar **mover dados para fora do Azure** (ex: estratégia multi-cloud, discutida na Aula 1) — não na migração inicial de on-premise para a nuvem.

e) **Como manter compliance LGPD** — onde os dados de brasileiros podem ficar?
R:

- **Região primária: Brazil South (São Paulo).** Dados de clientes brasileiros (CPF, endereço, histórico de compras) devem ser armazenados preferencialmente nessa região, atendendo ao princípio de minimização de transferência internacional da LGPD.
- **Replicação geográfica:** se for necessário DR (Disaster Recovery) com replicação para outra região (ex: East US 2), a QC deve avaliar se isso constitui "transferência internacional de dados" sob a LGPD e, se necessário, formalizar via cláusulas contratuais padrão (SCCs) com a Microsoft — que já oferece esses termos em seu Data Processing Addendum.
- **Dados sensíveis (CPF, dados de pagamento):** devem usar **Always Encrypted** no Azure SQL (criptografia client-side, onde nem o operador da nuvem acessa o dado em texto claro) e segredos de criptografia geridos via **Azure Key Vault** com HSM (Hardware Security Module).
- **Anonimização para analytics:** dados usados em treinamento de modelos de ML (Exercício 2.1, domínio "Modelos de ML") devem passar por pseudonimização antes de chegarem ao Data Lake, reduzindo o escopo de dados pessoais sujeitos à LGPD nas camadas analíticas.
...
a) Por que **NÃO** seria boa partitioning key:
   - `id` da review? (3 razões)
        ...
        1. **Fan-out total em queries por produto:** como cada review teria sua própria partição, buscar "todas as reviews do produto X" exigiria consultar todas as partições (cross-partition query), que é lento e caro em RU.

        2. **Sem agrupamento lógico:** o `id` é um valor único e arbitrário (geralmente um GUID), sem relação com o padrão de acesso real (que é "reviews por produto").

        3. **Overhead de gerenciamento:** o Cosmos criaria potencialmente milhões de partições lógicas com 1 item cada — desperdiçando a estrutura de particionamento sem trazer benefício de distribuição de carga real.


   - `score` (1-5)? (2 razões)

        1. **Cardinalidade extremamente baixa (apenas 5 valores possíveis):** isso cria "hot partitions" — toda review com score 5 (a maioria, tipicamente) cai na mesma partição física, concentrando RU/s e criando gargalo de throughput.

        2. **Tamanho da partição estoura o limite de 20 GB:** com 30M de reviews distribuídas em só 5 valores de partição, cada partição lógica facilmente ultrapassaria os 20 GB permitidos pelo Cosmos por partição lógica.


   - `data_da_review` (timestamp)? (2 razões)

        1. **Hot partition no presente:** todas as escritas recentes (novas reviews chegando agora) cairiam na mesma partição (a data de hoje), criando um gargalo de escrita — o oposto do objetivo de distribuir carga.

        2. **Queries por produto continuam ineficientes:** "todas as reviews do produto X" ainda exigiria cross-partition query, já que reviews do mesmo produto estariam espalhadas por múltiplas datas/partições.

b) Por que `produto_id` **funciona razoavelmente bem** mas pode ter um problema. Qual problema?
R:
`produto_id` funciona bem porque o **padrão de acesso dominante** é "ler todas as reviews de um produto específico" (ex: página de produto no e-commerce) — e o Cosmos resolve isso com uma única consulta dentro de uma única partição lógica, sem fan-out.

**O problema:** produtos "campeões de venda" (best-sellers) podem acumular **dezenas de milhares de reviews**, criando uma **partição quente (hot partition)** e potencialmente **excedendo os 20 GB de limite por partição lógica** — exatamente o cálculo pedido no item d). Produtos com poucas reviews ficam em partições pequenas e subutilizadas, gerando distribuição desigual de dados e de RU/s entre partições físicas.
...
R:
Para otimizar consultas por cliente **sem perder a eficiência das consultas por produto**, a QC pode usar **hierarchical partition keys** (disponível desde 2023 no Cosmos DB), definindo uma chave composta de até 3 níveis, por exemplo:
...
Isso permite:
- Consultas por `produto_id` continuam eficientes (primeiro nível da hierarquia já restringe a busca).
- Consultas que combinam `produto_id` + `cliente_id` (ex: "a review desse cliente sobre esse produto") atingem uma única partição física, com latência mínima.
- Para o caso "todas as reviews de um cliente, independente do produto", a hierarchical key sozinha não resolve — seria necessário um **índice secundário** (ex: um container separado ou um índice no Azure AI Search) que mapeie `cliente_id → lista de produto_ids/review_ids`, permitindo um lookup inicial seguido de consultas pontuais nas partições corretas.
...
Assumindo um tamanho médio por documento de review (título + conteúdo + metadados como score, data, cliente_id, produto_id) de aproximadamente **1 KB** (consistente com o limite de 1.01k caracteres do campo `content` observado na base Amazon Polarity):

```
Tamanho da partição = 50.000 reviews × 1 KB
                     = 50.000 KB
                     = ~48,8 MB

Quota por partição lógica do Cosmos: 20 GB = 20.480 MB

Percentual da quota utilizado:
  48,8 MB / 20.480 MB ≈ 0,24%
```

**Conclusão:** Para um produto com 50.000 reviews de ~1 KB cada, a partição usaria apenas **~0,24% da quota de 20 GB** — bem confortável. O problema descrito no item b) só se manifestaria em casos extremos: um produto precisaria de **mais de 20 milhões de reviews** (20.480 MB ÷ 1 KB) para estourar a quota — um cenário improvável mesmo para os maiores best-sellers da QC, mas que reforça a importância de **monitorar o crescimento das partições mais populares** ao longo do tempo, especialmente se o tamanho médio do documento crescer (ex: reviews com imagens anexadas ou respostas do vendedor).
